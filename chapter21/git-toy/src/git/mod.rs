...
#[cfg(windows)]
fn path_to_cstring(path: &Path) -> Result<CString> {
    // Try to convert to UTF-8. If fails, libgit2 can't handle the path
    // anyway.
    match path.to_str() {
...
/// database: a commit, tree, blob, tag, etc. This is wide hash of the
/// object's contents.
pub struct Oid {
    pub raw: raw::git_oid,
}
...
/// Try to borrow `&str`, given that `ptr` may be null or
/// refer to ill-formed UTF-8. Give the result a lifetime as if it were
/// borrowed from `_owner`.
///
/// Safety: if `ptr` is not-null, it must point to a null-terminated C
